...
import com.zeta.crm.dao.ModuleMapper;
...
public class ModuleService extends BaseService<Module,Integer> {
     ...
     *              非空，0|1|2
     *          模块名称moduleName
     *              非空，统一层级下模块名称唯一（不包含当前修改记录本身）
     *          地址URL
     *              二级菜单（grade=1），非空且统一层级下不可重复（不包含当前修改记录本身）
     *          权限码optValue
     *              非空，不可重复（不包含当前修改记录本身）
     *
     *      2.设置参数的默认值
    ...
    public void updateModule(Module module){
        ...
        //模块名称 moduleName 非空，同一层级下模块名称唯一（不包括当前修改记录本身）
        AssertUtil.isTrue(StringUtils.isBlank(module.getModuleName()),"模块名称不能为空！");
        //通过层级与模块名查询资源对象
        ...
        if(temp != null){
            AssertUtil.isTrue(!temp.getId().equals(module.getId()),"权限码已经存在！");
        }
